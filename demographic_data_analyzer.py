...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = int((num_min_workers.loc[df['salary'] == '>50K' ].shape[0] / num_min_workers.shape[0]) * 100)
    ...
    highest_earning_country_percentage = ((df[df['salary'] == '>50K'].groupby('native-country').size() / df.groupby('native-country').size()) * 100)[highest_earning_country].round(1)
    # Identify the most popular occupation for those who earn >50K in India.
    #aux = df.loc[ (df['salary'] == '>50K') & (df['native-country'] == 'India'),'occupation' ].value_counts().keys()[0]
    top_IN_occupation = df.loc[ (df['salary'] == '>50K') & (df['native-country'] == 'India'),'occupation' ].value_counts().keys()[0]
